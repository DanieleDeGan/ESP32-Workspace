...
| `tools/larghezza_testo.py` | quanto e' largo un testo **prima** di disegnarlo: somma gli `xAdvance` dei glifi nei `.h` veri dei font, come `getTextBounds()`. `--righe` e `--testata` rifanno le prove del layout della pagina nodi (aggiornate alla `v59`: il piede non esiste piu'). Serve perche' un testo troppo largo non da' errore — si sovrappone, e lo si scopre guardando il vetro |
| `tools/pannello_mock.py` | **`Adafruit_GFX` rifatto in Python**: disegna una pagina del pannello **senza la scheda**, sulla stessa tela 400x300 a 1 bit, con i font veri (glifi e bitmap dai `.h`), le icone di `icone.h` e le stesse primitive. Esce un PNG o i 15.000 byte del formato anteprima. Serve perche' le larghezze si potevano gia' misurare, ma **una riga che ci sta puo' lo stesso essere brutta** — il ritmo verticale e il grigio della pagina si vedono solo guardando, e prima costavano un OTA per volta. `--valida <ip>` (pagina nodi) e `--valida-dettaglio <ip>` fanno ridisegnare la scheda, ne scaricano l'anteprima e la confrontano **pixel per pixel**: e' la prova che il mock non stia mentendo, e va rifatta quando cambia il disegno nel `.ino` |
