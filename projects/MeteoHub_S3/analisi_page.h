...
//  (25988 byte di pagina, serviti su /analisi)
// ============================================================
 ...
 *{box-sizing:border-box}
 /* SERVE, e non e' pignoleria: [hidden]{display:none} lo mette il foglio di
    stile del browser, che ha priorita' MINORE di qualunque regola scritta
    qui. Con .barra{display:flex} le barre marcate `hidden` restavano
    visibili, con i select ancora vuoti — che e' esattamente come si e'
    presentato il difetto: "i selettori non mostrano i giorni". */
...
async function carica(){
 ...
 try{
  ...
  // La vista ATTIVA, non per forza quella iniziale: i riepiloghi arrivano da
  // due richieste in fila, e se nel frattempo si e' gia' cambiata scheda i
  // suoi selettori sono rimasti vuoti e non si riempirebbero piu' da soli.
  mostraVista(VISTA);
...
// Un elenco vuoto ha due cause diverse che vanno DETTE, non lasciate
// indovinare: i riepiloghi non sono ancora arrivati, oppure questo nodo non
// ne ha nessuno. Due select vuoti si somigliano e non spiegano niente.
function giorniPronti(){
 ...
 avviso(Object.keys(DATI).length
  ? 'Per <b>'+esc($('nodo').value)+'</b> non c&rsquo;&egrave; ancora nessun giorno chiuso: '+
    'l&rsquo;hub scrive una riga di riepilogo quando un giorno finisce.'
...
function mostraVista(v){
 ...
 else { riempiSelect('cA',g,g[g.length-1]); riempiSelect('cB',g,g[g.length-2]||g[0]); vistaConfronto(); }
}
